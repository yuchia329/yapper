...
class ProbeManifest(Artifact):
    ...
    width: int
    height: int
    rotation: int = 0  # display rotation (0/90/180/270); phone videos set this
    fps: float
    is_vfr: bool
    video_codec: str
    ...
    def display_width(self) -> int:
        """Width as DISPLAYED (encoded dims swap when rotated 90/270)."""
...
class Character(BaseModel):
    ...
    name: str = Field(
        description="Character name as spoken in dialogue (anchors identity)"
    )
...
class Beat(BaseModel):
    ...
    summary: str = Field(
        description="What happens in this story beat (plot, not narration)"
    )
    clip_refs: list[str] = Field(description="clip_ids whose footage covers this beat")
    est_spoken_seconds: float = Field(
        description="Rough narration time this beat needs"
    )
    importance: int = Field(
        description="1=droppable subplot ... 5=main throughline", ge=1, le=5
    )
...
class Screenplay(Artifact):
    ...
    logline: str = Field(description="One-sentence hook for the whole commentary")
    characters: list[Character] = Field(
        description="Main characters anchored to spoken names"
    )
    beats: list[Beat] = Field(description="Ordered story beats on the 主线 throughline")


# ---------------------------------------------------------------------------
# s07 — 解說 narration (Claude REDUCE pass)
# ---------------------------------------------------------------------------
...
class ScriptLine(BaseModel):
    ...
    kind: Literal["narration", "playback"] = Field(
        default="narration",
        description="narration=念解說词，配音盖在画面上；playback=放一小段原片原声，不配解說",
    )
    text: str = Field(
        description="narration: 要念出来的中文解說词；playback: 这段原片要显示的中文字幕（通常是原台词的翻译）"
    )
...
class EdlSegment(BaseModel):
    ...
    vo_in: float = (
        0.0  # offset into vo_file (for montage sub-segments sharing one line's voiceover)
    )
    vo_duration: (
        float  # this segment's screen time; for playback == the source window length
    )
    subtitle_text: str
